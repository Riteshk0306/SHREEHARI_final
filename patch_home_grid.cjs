...
const replacement = `<div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
            {featured.map(product => (
              <Link key={product.id} to={\`/products/\${product.id}\`} className="group block bg-white/95 backdrop-blur-sm rounded-xl shadow-sm border border-slate-200 overflow-hidden hover:shadow-md hover:border-amber-500 transition-all">
