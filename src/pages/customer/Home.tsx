...
export default function Home() {
  ...
  useEffect(() => {
    api.get('/api/products').then(data => setFeatured(data.slice(0, 4)));
  }, []);
      ...
      <div className="relative z-10">
        ...
        <div className="relative py-24 px-4 sm:px-6 lg:px-8 overflow-hidden">
          <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between">
            <div className="md:w-1/2 z-10">
              <h1 className="text-4xl md:text-6xl font-bold text-slate-900 leading-tight uppercase tracking-tight">
                Divine <span className="text-amber-500">Pooja Samagri</span> <br/> for your spiritual journey
              </h1>
              <p className="mt-6 text-lg text-slate-700 max-w-lg font-medium drop-shadow-sm">
                Authentic, premium quality pooja items delivered right to your doorstep. Experience purity in every prayer.
              </p>
              <div className="mt-8">
                <Link to="/products" className="bg-slate-900 text-white px-8 py-3.5 rounded-lg font-bold hover:bg-slate-800 transition shadow-lg shadow-slate-200 inline-block tracking-wide uppercase text-sm">
                  Shop Now
                </Link>
              </div>
            </div>
            <div className="md:w-1/2 mt-12 md:mt-0 relative">
              <img src="/shreeharikapoor.png" alt="Pooja Items" className="rounded-xl shadow-2xl relative z-10" referrerPolicy="no-referrer" />
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[120%] h-[120%] bg-amber-200 rounded-full blur-3xl opacity-30 -z-0"></div>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="flex justify-between items-end mb-8">
            <div>
              <h2 className="text-3xl font-bold text-slate-900 tracking-tight uppercase drop-shadow-sm">Featured Products</h2>
              <p className="text-slate-600 mt-2 font-medium drop-shadow-sm">Our most loved authentic items</p>
            </div>
            <Link to="/products" className="text-amber-600 font-bold hover:underline hidden sm:block uppercase tracking-wider text-sm drop-shadow-sm">View All</Link>
          </div>
